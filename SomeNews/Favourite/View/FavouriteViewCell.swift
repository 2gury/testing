...
class FavouriteViewCell: UITableViewCell {
    ...
    var favouriteButton: UIButton = {
        ...
        btn.setImage(filledImage, for: .normal)
        btn.setImage(emptyImage, for: .highlighted)
        return btn
    }()
...
//        labelStackView.rightAnchor.constraint(equalTo: self.rightAnchor).isActive = true
    ...
    private func setupFavouriteButton() {
        favouriteButton.addTarget(self, action: #selector(removeFromFavourite), for: .touchUpInside)
    ...
    @objc
    func removeFromFavourite(sender: UIButton) {
        CoreDataManager.shared.deleteProductFromFavourite(news: news)
